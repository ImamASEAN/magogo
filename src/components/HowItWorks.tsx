...
const steps = [
  ...
  { num: '04', icon: Package, title: 'Biomass Production', description: 'Harvested larvae are processed into premium biomass: high-protein animal feed, organic fertilizer, or feedstock for other industries.', detail: 'Animal feed · Fertilizer · Industrial feedstock', color: '#2d7a4f' },
  { num: '05', icon: RefreshCw, title: 'Circular Economy', description: 'Zero waste. The cycle continues — residual frass becomes fertilizer, creating a complete closed-loop system with no outputs going to landfill.', detail: '100% closed loop · Carbon positive · Scalable', color: '#3dbd6f' },
]
          ...
          {steps.map((step, i) => (
            <motion.div key={step.num}
              ...
              <div className="relative z-10 flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center"
                style={{ background: `linear-gradient(135deg, var(--color-forest), rgba(45,122,79,0.8))`, border: `2px solid ${step.color}40`, boxShadow: `0 0 24px ${step.color}25` }}>
                <step.icon size={22} style={{ color: step.color }} />
              </div>
              <div className="flex-1 rounded-2xl p-6 hover:shadow-lg transition-shadow duration-300"
                style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(255,255,255,0.9)', boxShadow: '0 4px 24px rgba(26,58,42,0.06)', backdropFilter: 'blur(20px)' }}>
                ...
                <p className="text-sm leading-relaxed mb-4" style={{ color: 'rgba(26,58,42,0.65)' }}>{step.description}</p>
